...
public class NewFolderButton extends AbstractWidget {
    ...
    protected void extractWidgetRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        if (hovered) {
            ...
            if (System.currentTimeMillis() - hoverStartTime >= 500) {
                this.setTooltip(Tooltip.create(Component.literal("New Folder")));
            } else {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (!(client.screen instanceof ResourceTreeScreen resourceTreeScreen)) return true;
